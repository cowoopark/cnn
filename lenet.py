...
with graph.as_default():
    ...
    c3_weights = tf.Variable(tf.truncated_normal([patch_size, patch_size, depth, depth], stddev=0.1))
    c3_biases = tf.Variable(tf.constant(1.0, shape=[depth * depth]))

    c5_weights = tf.Variable(tf.truncated_normal(
      [patch_size * patch_size * depth * depth, c5_layer], stddev=0.1))
...
# Model.
    def model(data):
        ...
        conv = tf.nn.depthwise_conv2d(pooling_layer, c3_weights, [1,1,1,1],padding='VALID')
